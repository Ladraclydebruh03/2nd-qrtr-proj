...
public class Knight extends Piece{
    ...
    public Knight(String n, int posX, int posY, int v, String c){
    ...
    public void Move (Board board, int newX, int newY){
        ...
        if(Math.abs(tileNum-newTileNum)==6||Math.abs(tileNum-newTileNum)==15||Math.abs(tileNum-newTileNum)==10||Math.abs(tileNum-newTileNum)==17){
    ...
    public void Capture(Board board, int newX, int newY){
        ...
        if((Math.abs(tileNum-newTileNum)==6||Math.abs(tileNum-newTileNum)==15||Math.abs(tileNum-newTileNum)==10||Math.abs(tileNum-newTileNum)==17)&& board.getPieceOnTile(newX, newY)!=null){
            ...
            if(enemyPiece.getName().equals("king")){
                ...
                if(thisDice>enemyDice){
                    ...
                    System.out.println(this.getName() + " won the dice roll");
                    ...
                    System.out.println(enemyPiece.getName() + " won the dice roll");
                    board.removePieceFromTile(currentX, currentY);
                }
                else{
                    while(thisCoin.equals(enemyCoin)){
                    ...
                    if(thisCoin.equals("heads")){
                        board.removePieceFromTile(newX, newY);
                        System.out.println(this.getName() + " won the coin flip");
                        board.removePieceFromTile(currentX, currentY);
                        if(this.getValue()>=enemyPiece.getValue())
                            board.placePieceOnTile(currentX, currentY, this);
                        else{
                        ...
                        board.removePieceFromTile(currentX, currentY);
                        System.out.println(enemyPiece.getName() + " won the coin flip");
                    }
                }
            }
        }

...
public class Pawn extends Piece{
    ...
    public Pawn(String n, int posX, int posY, int v, String c){
    ...
    public void Move (Board board, int newX, int newY){
        ...
        if(firstMove){
            if(Math.abs(tileNum-newTileNum)/8<=2 && Math.abs(tileNum-newTileNum)%8==0){
            ...
            if(Math.abs(tileNum-newTileNum)/8 == 1&& Math.abs(tileNum-newTileNum)%8==0){
                board.removePieceFromTile(currentX, currentY);
                tileNum = (newY - 1) * 8 + newX;
                board.placePieceOnTile(newX, newY, this);
            }
    ...
    public void Capture(Board board, int newX, int newY){
        ...
        if((newTileNum != tileNum-7 || newTileNum != tileNum-9)&& board.getPieceOnTile(newX, newY)!=null){
